...
// Phase 1 of the object-storage migration: db/media-storage.sql (additive columns,
// storage_orphans queue, GC triggers, the solution_site_image_ready view). The
// content_base64 columns are kept; the DROP is phase 5 (db/media-storage-drop.sql).
...
async function main(){const url=process.env.NEON_DATABASE_URL||process.env.DATABASE_URL||process.env.POSTGRES_URL;if(!url)throw Error('Missing database');const sql=neon(url);await sql.transaction(statements(fs.readFileSync('db/media-storage.sql','utf8')).map(statement=>sql.query(statement)));console.log('Media storage migration (phase 1) applied atomically.');}
main().catch(()=>{console.error('Media storage migration failed; inspect database access/schema without sharing secrets.');process.exitCode=1;});
