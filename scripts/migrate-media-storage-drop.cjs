...
// Phase 5 of the object-storage migration: db/media-storage-drop.sql. IRREVERSIBLE —
// drops the content_base64 columns. Run only after phase 3 backfill + phase 4 --verify
// pass and a Neon backup branch is taken. See db/media-storage-drop.sql.
...
async function main(){const url=process.env.NEON_DATABASE_URL||process.env.DATABASE_URL||process.env.POSTGRES_URL;if(!url)throw Error('Missing database');const sql=neon(url);await sql.transaction(statements(fs.readFileSync('db/media-storage-drop.sql','utf8')).map(statement=>sql.query(statement)));console.log('Media storage migration (phase 5, content_base64 dropped) applied atomically.');}
main().catch(()=>{console.error('Media storage drop migration failed; inspect database access/schema without sharing secrets.');process.exitCode=1;});
