...
export default async function BlogArticle({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <article className="pb-16 sm:pb-24">
    ...
    <header className="px-4 pt-5 sm:px-7 sm:pt-8 lg:px-10">
      <div className="mx-auto max-w-[1400px] overflow-hidden rounded-[28px] px-6 py-6 sm:rounded-[32px] sm:px-10 sm:py-7 lg:px-16 lg:py-8" style={{ background: tone.background, color: tone.foreground }}>
        ...
        <div className="mt-10 max-w-4xl sm:mt-12 lg:mt-14">
          <h1 className="text-3xl font-semibold leading-[0.98] tracking-[-0.055em] sm:text-4xl lg:text-[3.65rem]">{post.title}</h1>
    ...
    <div className="mx-auto grid max-w-[1240px] gap-12 px-6 py-16 lg:grid-cols-[250px_minmax(0,760px)] lg:justify-between lg:py-24">
          ...
          {post.sections.map((section, index) => <section id={`seccion-${index + 1}`} key={section.title} className="scroll-mt-32">
            <div className="flex items-start gap-5 border-t border-stone-200 pt-7 sm:gap-8">
              ...
              <div className="min-w-0 flex-1">
                <h2 className="text-3xl font-semibold leading-[1.02] tracking-[-0.045em] sm:text-5xl">{section.title}</h2>
                <div className="mt-8 space-y-7">{section.paragraphs.map((paragraph, paragraphIndex) => <p key={paragraph} className={`${paragraphIndex === 0 ? 'text-xl leading-[1.55] text-stone-700 sm:text-2xl' : 'text-lg leading-8 text-stone-600'}`}>{paragraph}</p>)}</div>
              </div>
            </div>
          </section>)}
        </div>
