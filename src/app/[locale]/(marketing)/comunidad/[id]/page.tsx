...
import { localizedCategoryDisplayLabel } from '@/lib/taxonomy';
...
export default async function PublicCollectionPage({params}:{params:Promise<{id:string;locale:string}>}){
 const {id,locale}=await params;if(!isSolutionId(id))notFound();
 const fullDict=await getDictionary(locale as Locale);const dict=fullDict.community;
 ...
 return <section className="mx-auto w-full max-w-[1500px] px-5 pb-24 pt-14 sm:px-10 sm:pt-20 lg:px-16"><Link href="/comunidad" className="mb-9 inline-flex items-center gap-2 text-sm text-stone-500"><ArrowLeft className="size-4"/>{dict.detail.back}</Link><header className="mb-10 border-b border-stone-200 pb-8"><div className="flex flex-col items-start justify-between gap-6 sm:flex-row"><div className="min-w-0 w-full sm:flex-1"><h1 className="break-words text-4xl font-semibold tracking-[-0.04em] sm:text-6xl">{collection.name}</h1>{collection.description&&<p className="mt-5 max-w-2xl whitespace-pre-wrap break-words leading-relaxed text-stone-500">{collection.description}</p>}<p className="mt-5 text-sm text-stone-500">{dict.byPrefix} {collection.curator} <span aria-hidden="true">·</span> {collection.count} {collection.count===1?dict.projectSingular:dict.projectPlural}</p></div><ShareCollection id={id} dict={dict.share}/></div><div className="mt-6 flex flex-wrap gap-2">{collection.categories.map(category=>{const tone=brandColors[solutionCategoryTones[category]??'blue'];return <Link key={category} href={`/comunidad?category=${encodeURIComponent(category)}`} style={{color:tone.solid,backgroundColor:tone.soft}} className="rounded-full px-3 py-2 text-xs">{localizedCategoryDisplayLabel(category,locale)}</Link>;})}</div></header>
 {collection.projects.length?<div className="grid gap-x-6 gap-y-10 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">{collection.projects.map(project=><ProjectPin key={project.key} project={project} locale={locale}><SaveProjectButton projectKey={project.key} dict={fullDict.solutionUi.save}/></ProjectPin>)}</div>:<p className="py-16 text-center text-stone-500">{dict.detail.noProjects}</p>}
 <CommunityActions id={id} initialLikes={collection.likes} initialSaves={collection.saves} initialCommentsCount={collection.comments} liked={collection.liked} saved={collection.saved} own={collection.own} comments={comments} dict={dict} locale={locale}/>
 <p className="mt-14 border-t border-stone-200 pt-5 text-xs text-stone-400">{dict.detail.footerNote}</p></section>;
}
