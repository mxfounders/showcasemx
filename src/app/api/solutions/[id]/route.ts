...
export async function PATCH(request:NextRequest, props:{params: Promise<{id:string}>}) {
 ...
 const authorized=await sql`SELECT account_id FROM solution_reviewers WHERE account_id=${account.id} AND disabled_at IS NULL`;if(!authorized.length)return failure('No tienes permiso para revisar postulaciones.',403);
 rows=await sql`WITH changed AS (UPDATE founder_solutions SET status=${String(body.decision)},published_at=CASE WHEN ${body.decision==='published'} THEN now() ELSE published_at END,published_data=CASE WHEN ${body.decision==='published'} THEN data ELSE published_data END,version=version+1,updated_at=now() WHERE id=${params.id} AND owner_id<>${account.id} AND version=${Number(body.version)} AND status='pending' AND EXISTS(SELECT 1 FROM solution_reviewers WHERE account_id=${account.id} AND disabled_at IS NULL) RETURNING *), event AS (INSERT INTO solution_events(solution_id,status,message,actor_id) SELECT id,status,${body.message.trim()},${account.id} FROM changed RETURNING id) SELECT * FROM changed`;
