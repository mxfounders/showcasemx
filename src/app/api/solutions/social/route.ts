...
export async function POST(request:NextRequest){
 ...
 if(body.action==='like'){
  ...
  if(!rows.length)return failure('No puedes interactuar con tu propia ficha o ya no está publicada.',409);const [{count}]=await sql`SELECT count(*)::int count FROM solution_likes WHERE solution_id=${id}`;return ok({active:!!rows[0].active,count:Number(count)});
 }
 if(body.action==='comment'){const details=communityComment(body);if(typeof body.commentId!=='string'||!isSolutionId(body.commentId))return failure('Comentario inválido.',400);if(!details)return failure('Escribe un nombre o alias y un comentario de hasta 500 caracteres.',400);if(!await securityLimit('solution-comment',account.id,10))return failure('Espera antes de comentar otra vez.',429);const rows=await sql`WITH target AS (SELECT id FROM founder_solutions WHERE id=${id} AND published_data IS NOT NULL),inserted AS (INSERT INTO solution_comments(id,solution_id,author_id,author_name,body) SELECT ${body.commentId},id,${account.id},${details.name},${details.comment} FROM target ON CONFLICT(id) DO NOTHING RETURNING id,created_at) SELECT id,created_at,true created FROM inserted UNION ALL SELECT c.id,c.created_at,false created FROM solution_comments c JOIN target t ON t.id=c.solution_id WHERE c.id=${body.commentId} AND c.author_id=${account.id} LIMIT 1`;if(!rows.length)return failure('La ficha ya no está publicada.',404);return ok({id:rows[0].id,createdAt:rows[0].created_at,created:!!rows[0].created});}
 if(body.action==='delete-comment'){if(typeof body.commentId!=='string'||!isSolutionId(body.commentId))return failure('Comentario inválido.',400);const rows=await sql`DELETE FROM solution_comments WHERE id=${body.commentId} AND author_id=${account.id} RETURNING id`;return rows.length?ok():failure('No puedes eliminar este comentario.',404);}
