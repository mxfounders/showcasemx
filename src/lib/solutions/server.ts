...
export async function isReviewer(id:string){const sql=solutionsSql();const rows=await sql`SELECT account_id FROM solution_reviewers WHERE account_id=${id}`;return rows.length>0;}
