...
export async function requestVerification(account:{id:string;email:string}){
 ...
 try{await sendEmail(account.email,'Confirma tu correo en shwcs',`Confirma que este correo es tuyo:\n${config.origin}/verify-email#token=${token}\n\nEl enlace caduca en 30 minutos. Si no lo solicitaste, ignora este correo.`);}catch{await sql`DELETE FROM auth_email_verifications WHERE token_hash=${hashToken(token)}`;throw Error('Delivery unavailable');}
}
