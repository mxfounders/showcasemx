import { previewCategories } from "./catalog-preview";
...
export function searchCatalog(query: string) {
  ...
  const unique = Array.from(new Map(previewCategories.flatMap(category => category.products).filter(product => product.website).map(product => [product.website, product])).values());
