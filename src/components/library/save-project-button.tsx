...
export function SaveProjectButton({projectKey,initialSaved}:{projectKey:string;initialSaved?:boolean}){
 ...
 return <div className="space-y-3"><button type="button" disabled={pending||loading} aria-pressed={saved} onClick={()=>void change()} style={actionButtonStyle} className={libraryButton}>{saved?<Check className="size-4" aria-hidden="true"/>:<Bookmark className="size-4" aria-hidden="true"/>}{loading?'Consultando…':pending?'Guardando…':saved?'Guardado':'Guardar proyecto'}</button>
