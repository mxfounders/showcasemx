...
export function BoardGallery({boards}:{boards:BuyerBoard[]}){
 const [query,setQuery]=useState('');const [visibility,setVisibility]=useState('all');const needle=normalizeText(query);const visible=boards.filter(board=>normalizeText(board.name).includes(needle)&&(visibility==='all'||(board.visibility??'private')===visibility));
