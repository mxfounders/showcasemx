...
export function CopyArticleLink({ dict }: { dict?: { linkCopied: string; copyLink: string } }) {
  ...
  }}>{copied ? <Check className="h-4 w-4 text-[#47785B]" /> : <LinkIcon className="h-4 w-4" />}{copied ? (dict?.linkCopied ?? 'Enlace copiado') : (dict?.copyLink ?? 'Copiar enlace')}</button>;
}
