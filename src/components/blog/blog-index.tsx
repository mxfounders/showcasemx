    ...
    <div className="mx-auto max-w-[1440px] px-5 pb-24 pt-10 sm:px-8 lg:px-12 lg:pt-16">
      <section className="mb-16 lg:mb-24">
        <div className="flex items-center justify-between mb-8 px-1">
          <h1 className="text-2xl sm:text-3xl font-semibold tracking-tight max-w-3xl">{dict?.heroTitleIndex || "Ideas para elegir, construir y operar mejores productos y servicios"}</h1>
          <div className="flex gap-2 shrink-0">
            <button onClick={() => scroll('left')} className="flex size-10 items-center justify-center rounded-full border border-stone-200 bg-white text-stone-600 transition-colors hover:border-[#365DC4] hover:text-[#365DC4]" aria-label={dict?.leftAria || "Izquierda"}><ArrowLeft className="size-4" /></button>
            <button onClick={() => scroll('right')} className="flex size-10 items-center justify-center rounded-full border border-stone-200 bg-white text-stone-600 transition-colors hover:border-[#365DC4] hover:text-[#365DC4]" aria-label={dict?.rightAria || "Derecha"}><ArrowRight className="size-4" /></button>
          </div>
        </div>
            ...
            return (
              <Link key={`featured-${post.slug}`} href={`/blog/${post.slug}`} className="group relative flex w-[85vw] max-w-[420px] shrink-0 snap-center snap-always flex-col overflow-hidden rounded-[32px] border border-stone-200 bg-white sm:w-[400px] shadow-sm transition-shadow hover:shadow-md">
                <div className="relative flex h-24 shrink-0 items-start p-6" style={{ background: tone.background, color: tone.foreground }}>
                  <span className="w-fit rounded-full bg-white/20 px-3 py-1.5 text-[10px] font-semibold uppercase tracking-widest backdrop-blur-md">{dict?.featuredLabel || "Destacado"}</span>
                </div>
      ...
      <div className="grid gap-8 lg:grid-cols-[230px_minmax(0,1fr)] lg:gap-12">
        <aside className="lg:sticky lg:top-28 lg:h-fit">
          <p className="flex items-center gap-2 border-b border-stone-200 pb-4 text-sm font-medium"><ListFilter className="size-4" />{dict?.filterAndSort || "Filtrar y ordenar"}</p>
          <label className="mt-5 block text-xs uppercase tracking-[0.16em] text-stone-400" htmlFor="blog-sort">{dict?.sortLabel || "Orden"}</label>
          <details className="group relative mt-2 w-fit lg:w-full">
            <summary id="blog-sort" className="selector-dropdown-trigger min-w-48 justify-between [&::-webkit-details-marker]:hidden lg:w-full"><span>{sort === 'newest' ? (dict?.sortNewest || 'Más recientes') : sort === 'oldest' ? (dict?.sortOldest || 'Más antiguos') : (dict?.sortTitle || 'Título A–Z')}</span><ChevronDown aria-hidden="true" className="size-4 transition-transform duration-200 group-open:rotate-180 motion-reduce:transition-none" /></summary>
            <div className="absolute left-0 top-[calc(100%+0.5rem)] z-30 w-56 overflow-hidden rounded-2xl border border-stone-200 bg-white p-2 shadow-[0_18px_45px_-20px_rgba(41,37,36,0.35)]">
              {([['newest', dict?.sortNewest || 'Más recientes'], ['oldest', dict?.sortOldest || 'Más antiguos'], ['title', dict?.sortTitle || 'Título A–Z']] as const).map(([value, label]) => <button key={value} type="button" aria-pressed={sort === value} onClick={(event) => { setSort(value); event.currentTarget.closest('details')?.removeAttribute('open'); }} className={`flex w-full items-center justify-between rounded-xl px-4 py-3 text-left text-sm transition-colors hover:bg-stone-100 focus-visible:outline focus-visible:outline-2 focus-visible:outline-[#365DC4] ${sort === value ? 'selector-menu-active' : ''}`}>{label}{sort === value && <Check aria-hidden="true" className="size-4" />}</button>)}
            </div>
          </details>
          <p className="mt-7 text-xs uppercase tracking-[0.16em] text-stone-400">{dict?.categoryLabel || "Categoría"}</p>
        ...
        <section id="blog-posts-top" className="scroll-mt-24">
          <div className="flex min-h-14 items-center justify-between gap-4 border-b border-stone-200 pb-4">
            <p className="text-sm text-stone-400">{posts.length} {posts.length === 1 ? (dict?.articleCount || 'artículo') : (dict?.articlesCount || 'artículos')}</p>
            <div className="ml-auto hidden selector-tabs w-fit rounded-full bg-stone-100 p-1 sm:flex" aria-label={dict?.viewAria || "Vista"}>
              <button type="button" aria-label={dict?.gridLabel || "Cuadrícula"} aria-pressed={view === 'grid'} onClick={() => setView('grid')} className="selector-tab flex items-center gap-2"><Grid2X2 className="h-4 w-4"/><span>{dict?.gridLabel || "Cuadrícula"}</span></button>
              <button type="button" aria-label={dict?.listLabel || "Lista"} aria-pressed={view === 'list'} onClick={() => setView('list')} className="selector-tab flex items-center gap-2"><List className="h-4 w-4"/><span>{dict?.listLabel || "Lista"}</span></button>
            </div>
            <ExpandingSearch label={dict?.searchPlaceholder || "Buscar artículos"} placeholder={dict?.searchPlaceholder || "Buscar artículos"} value={query} onChange={setQuery} />
          </div>
            ...
            <>
                  ...
                  return (
                    <Link key={post.slug} href={`/blog/${post.slug}`} className={`action-button group flex min-h-[470px] flex-col overflow-hidden rounded-[26px] border border-stone-200 bg-white ${view === 'list' ? 'sm:grid sm:min-h-56 sm:grid-cols-[250px_1fr]' : 'md:min-h-[510px]'}`}>
                      ...
                      <div className="flex flex-1 flex-col p-6 sm:p-7">
                        ...
                        <p className="mt-4 line-clamp-3 text-sm leading-relaxed text-stone-500">{post.excerpt}</p>
                        <span className="mt-auto flex items-center justify-between pt-7 text-sm font-medium text-[#365DC4]">{dict?.readArticle || "Leer artículo"} <ArrowRight className="h-4 w-4 button-arrow" /></span>
                      </div>
                    </Link>
                  );
                })}
              ...
              {totalPages > 1 && (
                <div className="mt-16 flex items-center justify-center gap-4 sm:gap-6 border-t border-stone-200 pt-10">
                  ...
                  >
                    <ArrowLeft className="size-4" /> {dict?.previousPage || "Anterior"}
                  </button>
