...
export function NewsletterForm({ dict }: { dict?: any }) {
  ...
  const t = dict || {
    titles: ['¿Qué te trae por aquí?', '¿Cuál es tu rol?', '¿A qué correo te escribimos?'],
    errors: {
      submit: 'No pudimos registrar tu suscripción. Inténtalo de nuevo.',
      submitCatch: 'No pudimos confirmar la suscripción. Reintenta; no duplicaremos tu registro.'
    },
    profiles: newsletterProfiles,
    roles: newsletterRoles,
    labels: { email: "Tu correo electrónico", purpose: "Usaremos tu correo, perfil y rol para enviarte novedades y avisos relevantes de shwcs.", consent: "Quiero recibir novedades y avisos de shwcs por correo.", honeypot: "Deja este campo vacío" },
    placeholders: { email: "tu@correo.com" },
    step: "Paso", of: "de", registering: "Registrando…", subscribeBtn: "Suscribirme", continueBtn: "Continuar", backBtn: "Anterior",
    successTitle: "Gracias por sumarte.", successDesc: "Recibimos tu solicitud. Si ya estabas en la lista, no duplicaremos tu registro.", successBtn: "Explorar soluciones"
  };
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (!response.ok || result.ok !== true) throw new Error(result.error || t.errors.submit);
      setSent(true);
    } catch (failure) {
      setError(failure instanceof Error && !['TimeoutError', 'TypeError', 'SyntaxError'].includes(failure.name) ? failure.message : t.errors.submitCatch);
  ...
  if (sent) return <div ref={panel}>
    ...
    <h2 ref={question} tabIndex={-1} className="text-3xl font-semibold tracking-tight focus:outline-none">{t.successTitle}</h2>
    <p className="mt-4 text-stone-600">{t.successDesc}</p>
    <Link href="/" style={actionButtonStyle} className="action-button mt-8 inline-flex items-center gap-2 rounded-full px-6 py-3 font-medium">{t.successBtn} <ArrowRight aria-hidden="true" className="size-4" /></Link>
  </div>;

  return <form onSubmit={submit} aria-label="Suscripción al newsletter" aria-busy={pending}>
    <div className="mb-8 flex items-center gap-4">
      <span className="shrink-0 text-xs tabular-nums text-stone-500" aria-live="polite">{t.step} {step + 1} {t.of} 3</span>
    ...
    <div ref={panel} className="min-h-[310px]">
      <h2 ref={question} id="newsletter-question" tabIndex={-1} className="mb-6 text-2xl font-medium tracking-tight text-stone-900 focus:outline-none sm:text-3xl">{t.titles[step]}</h2>
      {step === 0 && <fieldset aria-labelledby="newsletter-question" className="grid gap-3 sm:grid-cols-2">
        {t.profiles.map((option: any, index: number) => <label key={option.value} className="relative cursor-pointer">
      ...
      {step === 1 && <fieldset aria-labelledby="newsletter-question" className="grid gap-3 sm:grid-cols-2">
        {t.roles.map((option: any, index: number) => <label key={option.value} className="relative cursor-pointer">
      ...
      {step === 2 && <div>
        <label htmlFor="newsletter-email" className="sr-only">{t.labels.email}</label>
        <input id="newsletter-email" name="email" type="email" autoComplete="email" inputMode="email" required maxLength={254} value={email} onChange={event => setEmail(event.target.value)} placeholder={t.placeholders.email} aria-describedby="newsletter-purpose" className="w-full border-0 border-b-2 border-stone-300 bg-transparent px-0 py-4 text-2xl text-stone-900 outline-none transition-colors placeholder:text-stone-400 focus:border-[#365DC4] sm:text-3xl" />
        <p id="newsletter-purpose" className="mt-4 text-xs leading-relaxed text-stone-500">{t.labels.purpose}</p>
        <label className="mt-7 flex cursor-pointer items-start gap-3 text-sm leading-relaxed text-stone-600"><input name="consent" type="checkbox" required checked={consent} onChange={event => setConsent(event.target.checked)} className="mt-1 size-4 shrink-0 accent-[#365DC4]" />{t.labels.consent}</label>
      </div>}
    </div>
    <div hidden aria-hidden="true"><label>{t.labels.honeypot}<input name="company" tabIndex={-1} autoComplete="off" /></label></div>
    {error && <p role="alert" className="mb-5 text-sm leading-relaxed text-stone-700">{error}</p>}
    <div className="mt-8 flex items-center justify-between gap-4">
      <button type="submit" disabled={pending} style={actionButtonStyle} className="action-button inline-flex items-center justify-center gap-3 rounded-full px-6 py-3.5 text-sm font-medium disabled:cursor-wait disabled:opacity-60">{pending ? t.registering : step === 2 ? t.subscribeBtn : t.continueBtn}<ArrowRight aria-hidden="true" className="size-4" /></button>
      {step > 0 && <button type="button" disabled={pending} onClick={() => { setError(''); setStep(value => value - 1); }} className="inline-flex items-center gap-2 rounded-full px-3 py-2 text-sm text-stone-500 transition-colors hover:text-stone-900 focus-visible:outline focus-visible:outline-2 disabled:opacity-50"><ArrowLeft aria-hidden="true" className="size-4" />{t.backBtn}</button>}
    </div>
  </form>;
}
